...
O kit de desenvolvimento de software Android (Android SDK) é um pacote com diversas ferramentas utilizadas pelos desenvolvedores parar criar aplicativos para plataforma android de forma nativa. o SDK inclui as principais bibliotecas, emuladores e obfuscadores.
Embora o SDK possa ser usado por linha de comando, o método mais comum é usar um ambiente de desenvolvimento integrado (IDE). Neste projeto utilizaremos a IDE oficial da google, o Android Studio.
...
\subsubsubsection{BuildTypes}
\subsubsubsection{Dependencies}
No build.gradle, o campo dependencias nos permite baixar bibliotecas externas, adicionar arquivos jar ou modulos em nosso projeto Android. Neste projetos utilizaremos as dependencias abaixo.

 \begin{description}
   \item[MPAndroidChart] Criação de gráficos
   \item[Glide] Carregamento de imagens
   \item[SmartTabLayout] Tablayout centralizado 
   \item[Gson] Converter JSON em POJO
   \item[YouTubeAndroidPlayerApi] Carregamento de videos
   \item[Fabric] Monitorar erros e estatisticas 
   \item[Firebase] Persistencia de dados
   \item[Volley] consumo de APIs REST
   \item[Play-services] Serviços da google
 \end{description}
\section{Ambiente de desenvolvimento}
